...
export class ItemRepository {
  ...
  async create(itemCreateInput: ItemCreateInput): Promise<SingleItem> {
    ...
    return {
      resource: await this.databaseService.item.create({
  ...
  async getAll(queryArgs: GetItemsQueryArgs): Promise<PaginatedItems> {
    ...
    const { category, limit = 10, page = 1 } = queryArgs;
  ...
  async getById(itemId: string): Promise<SingleItem> {
    ...
    return {
      resource: await this.databaseService.item.findUnique({
        where: {
          id: itemId,
        },
      }),
  ...
  async delete(itemId: string): Promise<SingleItem> {
    ...
    return {
      resource: await this.databaseService.item.delete({
        where: {
          id: itemId,
        },
      }),
